...
from utils.rna_cdm import (
    GaussianDiffusion,
    GaussianDiffusionContinuousTimes
)

from utils.unet import Unet, NullUnet
...
class ElucidatedRNACDM(nn.Module):
    def __init__(
        self,
        unets,
        *,
        ...
        rna_embed_dim = None,
        channels = 3,
        timesteps= 1000,                            # number of steps to use on Gaussian Schedu
        cond_drop_prob = 0.1,
        ...
        self.lowres_noise_schedule = GaussianDiffusionContinuousTimes(noise_schedule = lowres_noise_schedule, timesteps = timesteps)
    ...
    def force_unconditional_(self):
        self.condition_on_rna = False
        self.unconditional = True

        for unet in self.unets:
            unet.cond_on_rna = False
   ...
   # preconditioned network output
    ...
    def sample(
        ...
        if not self.unconditional:
            assert exists(rna_embeds), 'rna must be passed in if the network was not trained without text `condition_on_text` must be set to `False` when training'
        ...
        if exists(inpaint_images):
            ...
            assert not (self.condition_on_rna and inpaint_images.shape[0] != rna_embeds.shape[0]), 'number of inpainting images must be equal to the number of text to be conditioned on'
    ...
    def forward(
        ...
        **kwargs
        ...
        #print("images.shape L747: ", images.shape)
        ...
        #print("batch_size L765: ", batch_size)
        ...
        # print("images.shape L773: ", images.shape)
        check_shape(images, 'b c ...', c = self.channels)

        assert h >= target_image_size and w >= target_image_size

        if not self.unconditional:
            # rna_masks = default(rna_masks, lambda: torch.any(rna_embeds != 0., dim = -1))
            rna_masks = None
        # print("rna_embeds: ", rna_embeds)
        ...
        assert not (exists(rna_embeds) and rna_embeds.shape[-1] != self.rna_embed_dim), f'invalid text embedding dimension being passed in (should be {self.rna_embed_dim})'
        ...
        # get the sigmas
        #print("batch_size L841: ", batch_size)
        sigmas = self.noise_distribution(hp.P_mean, hp.P_std, batch_size)
        padded_sigmas = self.right_pad_dims_to_datatype(sigmas)

        # noise
        # print("images.shape, L845: ", images.shape)
        # print("sigmas.shape, L846: ", sigmas.shape)
        # print("padded_sigmas.shape, L847: ", padded_sigmas.shape)
