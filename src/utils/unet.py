...
class PerceiverResampler(nn.Module):
    ...
    def forward(self, x, mask = None):
        ...
        #print("n", n)
        #print("Embedding weight size:", self.pos_emb.weight.size())
...
class Unet(nn.Module):
    def __init__(
        ...
        *,
        ...
        # uni encoding conditioning (optional)

        self.uni_to_cond = None

        if cond_on_uni:
            assert exists(uni_embed_dim), 'uni_embed_dim must be given to the unet if cond_on_uni is True'
            self.uni_to_cond = nn.Linear(uni_embed_dim, 512)

        # finer control over whether to condition on uni encodings

        self.cond_on_uni = cond_on_uni

        # attention pooling

        self.attn_pool = PerceiverResampler(dim = cond_dim, depth = 2, dim_head = attn_dim_head, heads = attn_heads, num_latents = attn_pool_num_latents) if attn_pool_uni else None

        # for classifier free guidance

        self.max_uni_len = max_uni_len

        self.null_uni_embed = nn.Parameter(torch.randn(1, max_uni_len, cond_dim))
        self.null_uni_hidden = nn.Parameter(torch.randn(1, time_cond_dim))

        # for non-attention based text conditioning at all points in the network where time is also conditioned

        self.to_uni_non_attn_cond = None

        if cond_on_uni:
            self.to_uni_non_attn_cond = nn.Sequential(
    ...
    def cast_model_parameters(
        ...
        *,
        ...
        if lowres_cond == self.lowres_cond and \
            ...
            cond_on_uni == self.cond_on_uni and \
            uni_embed_dim == self._locals['uni_embed_dim'] and \
    ...
    def forward(
        ...
        *,
        ...
        if exists(uni_embeds) and self.cond_on_uni:

            # conditional dropout
            uni_keep_mask = prob_mask_like((batch_size,), 1 - cond_drop_prob, device = device)
        
            uni_keep_mask_embed = rearrange(uni_keep_mask, 'b -> b 1 1')
            uni_keep_mask_hidden = rearrange(uni_keep_mask, 'b -> b 1')
            uni_tokens = self.uni_to_cond(uni_embeds)
        
            uni_tokens = uni_tokens[:, :self.max_uni_len]  # Truncate to max_uni_len
        
            if exists(uni_mask):
                uni_mask = uni_mask[:, :self.max_text_len]
        
            uni_tokens_len = uni_tokens.shape[1]
            remainder = self.max_uni_len - uni_tokens_len
            if remainder > 0:
                uni_tokens = F.pad(uni_tokens, (0, 0, remainder, 0))  # Pad only the sequence length
        
            if exists(uni_mask):
                if remainder > 0:
                    uni_mask = F.pad(uni_mask, (remainder, 0), value=False)
        
                uni_mask = rearrange(uni_mask, 'b n -> b n 1')
                uni_keep_mask = uni_mask & uni_keep_mask_embed
        
            null_uni_embed = self.null_uni_embed.to(uni_tokens.dtype)  # Match dtype for AMP
            uni_tokens = rearrange(uni_tokens, 'b n -> b 1 n')  # Keep batch size intact

            uni_tokens = torch.where(
                uni_keep_mask_embed,
                uni_tokens,
                null_uni_embed
            )
            ...
            if exists(self.attn_pool):
                uni_tokens = self.attn_pool(uni_tokens)
        ...
        c = time_tokens if not exists(uni_tokens) else torch.cat((time_tokens, uni_tokens), dim = -2)
