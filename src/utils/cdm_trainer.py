...
from utils.unet import NullUnet
...
class CRAWFORDTrainer(nn.Module):
    locked = False
    ...
    def __init__(
        ...
        precision = None,
        split_batches = True,
        dl_tuple_output_keywords_names = ('images', 'uni_embeds', 'cond_images'),
        verbose = True,
        ...
        **kwargs
        ...
        super().__init__()
        assert not CRAWFORDTrainer.locked, 'CRAWFORDTrainer can only be initialized once per process - for the sake of distributed training, you will now have to create a separate script to train each unet (or a script that accepts unet number as an argument)'
        assert exists(imagen) ^ exists(imagen_checkpoint_path), 'either CRAWFORD instance is passed into the trainer, or a checkpoint path that contains the imagen config'
        ...
        assert isinstance(imagen, (CRAWFORD))
        ...
        CRAWFORDTrainer.locked = self.is_distributed
    ...
    def validate_and_set_unet_being_trained(self, unet_number = None):
        ...
        #print("unet_number", unet_number)
        #print("self.only_train_unet_number", self.only_train_unet_number)
    ...
    def load(self, path, only_model = False, strict = True, noop_if_not_exist = False):
        ...
        self.global_steps = loaded_obj['global_steps']
    ...
    def save(
        ...
        **kwargs
        ...
        save_obj = dict(
            model=self.imagen.state_dict(),
            steps=self.steps.cpu(),  # Already present
            global_steps=self.global_steps,
            last_batch_index=self.current_batch_index,  # Track the last batch index
            **kwargs
        )
    ...
    def load(self, path, only_model=False, strict=True, noop_if_not_exist=False):
        ...
        self.current_batch_index = loaded_obj.get('last_batch_index', 0)
